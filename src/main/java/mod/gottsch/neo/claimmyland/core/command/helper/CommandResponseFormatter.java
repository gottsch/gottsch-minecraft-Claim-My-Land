...
import static mod.gottsch.neo.claimmyland.core.command.helper.FormatterConstants.newline;
...
//        return buildFull(ChatFormatting.GREEN, "✔", titleKey, bodyKey, bodyArgs);
    ...
    private static List<Component> buildShorthand(
        ...
        List<Component> lines = new ArrayList<>();
        MutableComponent title = Component.translatable(LangUtil.chat(langKey), coerceArgs(args))
...
//        }
        ...
        lines.add(
                Component.translatable(LangUtil.chat(bodyKey), (Object[]) coerceArgs(bodyArgs))
//                Component.translatable(LangUtil.chat(bodyKey), bodyArgs)
    ...
    private static Object[] coerceArgs(Object... args) {
        Object[] out = new Object[args.length];
        for (int i = 0; i < args.length; i++) {
            Object a = args[i];
            if (a == null) {
                out[i] = Component.empty();
            } else if (a instanceof Component || a instanceof String
                    || a instanceof Number || a instanceof Boolean) {
                out[i] = a;
            } else {
                out[i] = Component.literal(a.toString());
            }
        }
        return out;
    }
